...
import { saveContract, updateContract, getContract } from '../api.ts'
...
export default {
  ...
  setup(props: any, { emit }: any) {
    ...
    onMounted(async () => {
      ...
      dicts.customers = customerOptions.map(el => ({ value: el.id, label: el.name }))

      const { data: goodsOptions } = await getAllGoods()
      dicts.goods = goodsOptions.map(el => ({ value: el.id, label: el.name }))
